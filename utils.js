module.exports = {
   ...
   *
   * @param {Creep} creep
   * @param {number} requiredDamage
   */
  getRequiredDamageFor(creep, requiredDamage, options = {}) {
    const { assumeFull } = { assumeFull: false, ...options }
    ...
    for (const part of creep.body) {
      ...
      if (part.boost && BOOSTS[part.type][part.boost] && BOOSTS[part.type][part.boost].damage) {
        damageRatio = BOOSTS[part.type][part.boost].damage
      }

      const hits = assumeFull ? 100 : part.hits

      damage += Math.min(hits, requiredDamage) / damageRatio

      requiredDamage -= Math.min(hits, requiredDamage)
    }
